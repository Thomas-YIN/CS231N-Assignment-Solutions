...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = (np.exp(out)-np.exp(-out))/(np.exp(out)+np.exp(-out))
...
def rnn_step_backward(dnext_h, cache):
    ...
    # dtanh: (N, H)
    dtanh = 1 - ((np.exp(out) - np.exp(-out)) ** 2)/((np.exp(out) + np.exp(-out)) ** 2)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c_tanh = 1 - next_c_tanh**2
 
    # gradients w.r.t. f * prev_c + i * g 
    dnc_prod = dnext_h * o * dnext_c_tanh + dnext_c
    # di
    dgate[:,:H] *= dnc_prod * g
    # df
    ...
    dx = dgate @ Wx.T
    dprev_h = dgate @ Wh.T
    dprev_c = dnext_c * f + dnext_h * o * dnext_c_tanh * f
    dWx = x.T @ dgate
    dWh = prev_h.T @ dgate
